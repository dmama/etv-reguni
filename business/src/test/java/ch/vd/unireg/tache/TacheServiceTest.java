...
public class TacheServiceTest extends BusinessTest {
	...
	public void testGenereTacheEmissionDiApresDeces() throws Exception {
		...
		final CollectiviteAdministrative aciSuccessions = doInNewTransaction(status -> tiersService.getCollectiviteAdministrative(MockCollectiviteAdministrative.noNouvelleEntite));
	...
	public void testGenerateTacheMariagePeriodePassee() throws Exception {
		...
		// après 2007, on ne trouve pas de DI et on ne considère *pas* le contribuable (ménage) comme nouvel assujetti (n-1 a un assujettissement ordinaire), donc VAUD_TAX
		for (int i = 2008; i < today.year(); ++i) {
			...
			assertTache(TypeEtatTache.EN_INSTANCE, dateEcheance, date(i, 1, 1), date(i, 12, 31), TypeContribuable.VAUDOIS_ORDINAIRE, TypeDocument.DECLARATION_IMPOT_VAUDTAX,
	...
	public void testGenerateTacheDivorcePeriodePassee() throws Exception {
		...
		// Monsieur
		{
			...
			// après 2007, on ne trouve pas de DI et on ne considère pas le contribuable comme nouvel assujetti (n-1 a un assujettissement ordinaire), donc VAUD_TAX
...
//			assertTache(TypeEtatTache.EN_INSTANCE, nextSunday, nouveaux.get(0));
	...
	public void testGenereTacheDepuisFermetureForPrincipalUNIREG1303() throws Exception {
		...
		doInNewTransaction(status -> {
			...
			for (int i = 2003; i <= 2008; i++) {
				PeriodeFiscale periode = pfDAO.getPeriodeFiscaleByYear(i);
				ModeleDocument modele = addModeleDocument(TypeDocument.DECLARATION_IMPOT_COMPLETE_BATCH, periode);
				addDeclarationImpot(raoul, periode, date(i, 1, 1), date(i, 12, 31), TypeContribuable.VAUDOIS_ORDINAIRE, modele);
			}
	...
	private void sortTachesEnvoi(final List<? extends TacheEnvoiDeclarationImpot> envois) {
		envois.sort(Comparator.comparing(TacheEnvoiDeclarationImpot::getDateDebut));
	}

	private void sortTachesAnnulation(final List<TacheAnnulationDeclarationImpot> annulations) {
		annulations.sort(Comparator.comparing(o -> o.getDeclaration().getDateDebut()));
	}
	...
	public void testDetermineSynchronizeActionsAnnulationForIndigentEtRemplacementForOrdinaire() throws Exception {
		...
		doInNewTransactionAndSession(status -> {
			...
			addForPrincipal(pp, date(2004, 3, 1), MotifFor.INDETERMINE, date(2008, 12, 31), MotifFor.CHGT_MODE_IMPOSITION, MockCommune.Aigle, ModeImposition.INDIGENT);
			addForPrincipal(pp, date(2009, 1, 1), MotifFor.CHGT_MODE_IMPOSITION, MockCommune.Aigle);
	...
	public void testRecalculTachesAvecDiSansTypeContribuable() throws Exception {
		...
		// mise en place
		final long ppId = doInNewTransaction(transactionStatus -> {
	...
	public void testDetermineSynchronizeActionsForDIsDecesAvecRattrapage() throws Exception {
		...
		assertTache(TypeEtatTache.EN_INSTANCE, dateDeces.addDays(30), date(anneePrecedente, 1, 1), dateDeces, TypeContribuable.VAUDOIS_ORDINAIRE, TypeDocument.DECLARATION_IMPOT_VAUDTAX,
	...
	public void testTypeDeclarationImpotDansTacheEnvoiPP() throws Exception {
		...
		doInNewTransactionAndSession(status -> {
			...
			for (Tache tache : taches) {
				...
				// [SIFISC-29906] dit ceci:
				// la première année, pas de DI avant, ni assujettissement -> VAUDTAX
				// la seconde année, toujours pas de DI avant, mais assujettissement l'année précédente -> VAUDTAX
				final TypeDocument typeDocumentAttendu = TypeDocument.DECLARATION_IMPOT_VAUDTAX;
	...
	public void testTacheAutomatiqueEnvoiPM() throws Exception {
		...
		try {
			// mise en place fiscale
			pmId = doInNewTransactionAndSession(status -> {
	...
	public void testSynchronisationTacheEnvoiDiBatchSurDeclarationExistanteLocale() throws Exception {
		...
		try {
			...
			// mise en place fiscale
			pmId = doInNewTransactionAndSessionUnderSwitch(tacheSynchronizer, false, status -> {
				// l'entreprise
