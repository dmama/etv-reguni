package ch.vd.unireg.evenement.entreprise.casmetier.doublon;
...
public class DoublonsTest extends AbstractEvenementEntrepriseCivileProcessorTest {
	...
	public void testEvenementDoublonEntrepriseRemplacee() throws Exception {
		...
		final Long etablissementRemplaçantId = doInNewTransactionAndSession(status -> {
			final Etablissement etablissement = addEtablissement();
			etablissement.setNumeroEtablissement(noEtablissementRemplacant);
			return etablissement.getNumero();
		});
		final Long entrepriseRemplaçanteId = doInNewTransactionAndSession(status -> {
	...
	public void testEvenementDoublonEntrepriseVuDuRemplacant() throws Exception {
		...
		final Long etablissementRemplaceeParId = doInNewTransactionAndSession(status -> {
			final Etablissement etablissement = addEtablissement();
			etablissement.setNumeroEtablissement(noEtablissementRemplaceePar);
			return etablissement.getNumero();
		});
		final Long entrepriseRemplaceeParId = doInNewTransactionAndSession(status -> {
	...
	public void testEvenementDoublonEtablissementInconnu() throws Exception {
		...
		final Long tiersId = doInNewTransactionAndSession(status -> addEntrepriseConnueAuCivil(noEntrepriseCivile).getNumero());
		...
		doInNewTransactionAndSession(status -> {
			...
			Assert.assertEquals(
					String.format("Traitement manuel requis: Doublon d'établissement civil à l'IDE. L'établissement non encore connue d'Unireg (civil: %d) est remplacé par l'établissement non encore connue d'Unireg (civil: %d).", noEtablissement,
					              noEtablissementRemplacant),
					evt.getErreurs().get(2).getMessage());
			return null;
		});
	}
