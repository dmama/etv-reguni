...
public class DepartTest extends AbstractEvenementCivilInterneTest {
	...
	public void testDepartSecondaireDeCommuneDestinationInconnuePersonnePhysiqueSansFor() throws Exception {
		...
		serviceCivil.setUp(new DefaultMockIndividuConnector(false) {
			...
			protected void init() {
				...
				adresse.setLocalisationSuivante(new Localisation(LocalisationType.HORS_SUISSE, InfrastructureConnector.noPaysInconnu, null));
			}
		});
	...
	public static boolean findEvenementFermetureFor(Collection<EvenementFiscal> lesEvenements, Depart depart) {
		...
		while (iteEvFiscal.hasNext()) {
			...
			if (evenement instanceof EvenementFiscalFor && evenement.getDateValeur() == depart.getDate() && ((EvenementFiscalFor) evenement).getType() == EvenementFiscalFor.TypeEvenementFiscalFor.FERMETURE) {
				isPresent = true;
				break;
			}
		}
