	...
	private List<IdentifiantDeclaration> getQuestionnairesARappeler(final RegDate dateTraitement, @Nullable final Integer periodeFiscale) {
		...
		return template.execute(status -> {
			final List<Object[]> rows = hibernateTemplate.execute(session -> {
				...
				b.append(" AND EXISTS (SELECT etat.declaration.id FROM EtatDeclaration AS etat WHERE qsnc.id = etat.declaration.id AND etat.annulationDate IS NULL AND type(etat) = EtatDeclarationEmise)");
				b.append(
						" AND NOT EXISTS (SELECT etat.declaration.id FROM EtatDeclaration AS etat WHERE qsnc.id = etat.declaration.id AND etat.annulationDate IS NULL AND type(etat) IN (EtatDeclarationRetournee, EtatDeclarationSommee, EtatDeclarationRappelee, EtatDeclarationSuspendue, EtatDeclarationEchue))");
