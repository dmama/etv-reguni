 ...
 *
 * Test de non regression de la methode GetListeCtbModifies
 *
 *
 * @author xsibnm
 * @version $Revision: 1.0 $
 */
public class ServiceFiscalGetListeCtbModifiesTestCase extends TestCase {
    ...
    private Runnable createRunnable(final int index) {
        return new Runnable() {
            public void run() {
                try {
                    // 10000083 ou 99919903 : Utilise le plus grand possible pour éviter de lire toute la table;
                    int nextCtbNum = 99919903;

                    GregorianCalendar calDebRech = new GregorianCalendar(2005, Calendar.DECEMBER, 25);
                    Date datDebRech = calDebRech.getTime();

                    GregorianCalendar calFinRech = new GregorianCalendar(2005, Calendar.DECEMBER, 26);
                    Date datFinRech = calFinRech.getTime();

                    getServiceFiscal().getListeCtbModifies(datDebRech, datFinRech, nextCtbNum);
                } catch (Exception e) {
