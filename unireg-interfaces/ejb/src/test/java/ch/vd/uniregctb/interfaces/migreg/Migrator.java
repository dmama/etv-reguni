...
public  class Migrator{
	...
	public void migreSchumi() throws Exception{
		MigRegLimitsList limitsList = new MigRegLimitsList("MigregUniregInterface");
		{
			MigRegLimits limit = new MigRegLimits();
			limit.setOrdinaire(10236988, 10236988);
			limit.setWantCouple(false);
			limitsList.append(limit);
		}
		{
			MigRegLimits limit = new MigRegLimits();
			limit.setOrdinaire(10236989, 10236989);
			limit.setWantCouple(false);
			limitsList.append(limit);
		}
		{
			MigRegLimits limit = new MigRegLimits();
			limit.setOrdinaire(87007406, 87007406);
			limit.setWantCouple(true);
			limitsList.append(limit);
		}
		executeMigration(limitsList, 20);
	...
	public void migreDonneesDevel() throws Exception{
		executeMigration(LimitsConfigurator.cfg(LimitsConfigurator.DEVEL, null), 10);
	}
}
