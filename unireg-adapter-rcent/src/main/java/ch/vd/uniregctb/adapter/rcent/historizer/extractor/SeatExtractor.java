...
public class SeatExtractor implements Function<Organisation, Stream<Keyed<BigInteger, Integer>>> {

	@Override
	public Stream<Keyed<BigInteger, Integer>> apply(Organisation org) {
		return org.getOrganisationLocation().stream()
				.filter(ol -> ol.getSeat() != null)
				.map(ol -> new Keyed<>(ol.getCantonalId(), ol.getSeat().getMunicipalityId()));
	}
}