 ...
 *
 * Fonctions {@link EntrepriseActiviteHelper#isActif} et {@link EntrepriseActiviteHelper#activite} déplacées de {@link EntrepriseHelper} le 2017-08-14.
 *
 * @author Raphaël Marmier, 2017-08-14, <raphael.marmier@vd.ch>
 */
public class EntrepriseActiviteHelper {

	private static final Logger LOGGER = LoggerFactory.getLogger(EntrepriseActiviteHelper.class);

	/**
	 * <p>
	 *     Détermine la plage d'activité d'une entreprise, c'est à dire la période à laquelle on la considère en existence au sens de
	 ...
	 *     <ul>
	 *         <li>
	 *             Les entreprises radiées puis réinscrites sont considérées comme actives durant toutes la période ou elles
	 ...
	 *             au RC pour les entreprises obligatoirement inscrites au RC), c'est la date du RC qui est utilisée. Actuellement,
	 *             le seuil {@link EntrepriseHelper#NB_JOURS_TOLERANCE_DE_DECALAGE_RC} est utilisé pour deviner ce cas de figure. Ainsi,
	...
	public static List<DateRange> activite(EtablissementCivil etablissement) {
		...
		if (first == null) {
			dateCreation = etablissement.getNom().get(0).getDateDebut(); // Postulat: une entreprise commence toujours par être active. (pas d'inscription en inactivité)(attention REE est-ce toujours vrai?)
		}
...
*/
		...
		if (periodesStatusIde != null && !periodesStatusIde.isEmpty()) {
			...
			if (dernierePeriode.getDateFin() == null && (status == StatusRegistreIDE.RADIE || status == StatusRegistreIDE.DEFINITIVEMENT_RADIE)) {
				...
				if (dateRadiationRC != null && !dateRadiationRC.isBefore(dernierePeriode.getDateDebut().addDays( - EntrepriseHelper.NB_JOURS_TOLERANCE_DE_DECALAGE_RC))) {
	...
	public static boolean isActif(EtablissementCivil etablissement, RegDate date) {
		if (EntrepriseHelper.isConnuInscritAuRC(etablissement, date) && !EntrepriseHelper.isRadieDuRC(etablissement, date)) {
			return true;
		}
		else if (EntrepriseHelper.isInscritIDE(etablissement, date) && !EntrepriseHelper.isRadieIDE(etablissement, date)) {
			return true;
		}
		else if (EntrepriseHelper.isConnuInscritREE(etablissement, date) && !EntrepriseHelper.isRadieREE(etablissement, date)) {
			return true;
		}
		return false;
	}
