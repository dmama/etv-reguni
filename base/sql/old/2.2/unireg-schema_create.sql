...
create table MIGREG_ERROR (id number(19,0) not null, FOR_PRINCIPAL_CTB varchar2(255 char), LIBELLE_MESSAGE varchar2(255 char), NO_CONTRIBUABLE number(19,0) unique, NO_INDIVIDU number(10,0) unique, NOM_CONTRIBUABLE varchar2(255 char), NOM_INDIVIDU varchar2(255 char), TYPE_ERREUR varchar2(255 char), primary key (id));

create table MODELE_DOCUMENT (MODELE_TYPE varchar2(31 char) not null, id number(19,0) not null, ANNULATION_DATE timestamp, ANNULATION_USER varchar2(255 char), LOG_CDATE timestamp, LOG_CUSER varchar2(255 char), LOG_MDATE timestamp, LOG_MUSER varchar2(255 char), TYPE_DOCUMENT varchar2(29 char), PERIODE_ID number(19,0), primary key (id));

create table MODELE_FEUILLE_DOC (id number(19,0) not null, ANNULATION_DATE timestamp, ANNULATION_USER varchar2(255 char), LOG_CDATE timestamp, LOG_CUSER varchar2(255 char), LOG_MDATE timestamp, LOG_MUSER varchar2(255 char), INTITULE_FEUILLE varchar2(255 char), NOM_DOC varchar2(255 char), FEUILLE_ID number(19,0), primary key (id));
...
create table PERIODE_FISCALE (id number(19,0) not null, ANNULATION_DATE timestamp, ANNULATION_USER varchar2(255 char), LOG_CDATE timestamp, LOG_CUSER varchar2(255 char), LOG_MDATE timestamp, LOG_MUSER varchar2(255 char), ANNEE number(10,0) unique, DATE_FIN_ENVOI_MASSE number(10,0) not null, TERME_GEN_SOMM_EFFECT number(10,0), TERME_GEN_SOMM_REGL number(10,0), primary key (id));

create table RAPPORT_ENTRE_TIERS (RAPPORT_ENTRE_TIERS_TYPE varchar2(31 char) not null, id number(19,0) not null, ANNULATION_DATE timestamp, ANNULATION_USER varchar2(255 char), LOG_CDATE timestamp, LOG_CUSER varchar2(255 char), LOG_MDATE timestamp, LOG_MUSER varchar2(255 char), DATE_DEBUT number(10,0), DATE_FIN number(10,0), TYPE varchar2(20 char), DATE_FIN_DER_ELE_IMP number(10,0), TAUX_ACTIVITE number(10,0), TYPE_ACTIVITE varchar2(14 char), EXTENSION_EXECUTION_FORCEE number(1,0), TIERS_TUTEUR_ID number(19,0), TIERS_SUJET_ID number(19,0), TIERS_OBJET_ID number(19,0), primary key (id));
...
create index IDX_FF_CTB_ID on FOR_FISCAL (CTB_ID);

alter table FOR_FISCAL add constraint FK_FF_CTB_ID foreign key (CTB_ID) references TIERS;

alter table IDENTIFICATION_PERSONNE add constraint FK_ID_PERS_TRS_ID foreign key (NON_HABITANT_ID) references TIERS;

alter table MODELE_DOCUMENT add constraint FK_MODOC_PF_ID foreign key (PERIODE_ID) references PERIODE_FISCALE;

alter table MODELE_FEUILLE_DOC add constraint FK_FLLE_MODOC_ID foreign key (FEUILLE_ID) references MODELE_DOCUMENT;
