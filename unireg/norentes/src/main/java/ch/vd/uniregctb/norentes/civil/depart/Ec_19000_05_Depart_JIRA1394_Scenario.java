	...
	public void check1() {

		{
			...
			assertEquals(commune.getNomMinuscule(), 
					serviceCivilService.getAdresses(noIndSven, RegDate.get()).principale.getLocalite(), 
		...
		
		{
			...
			// vérification que les adresses civiles sont à Vallorbe
			assertEquals(commune.getNomMinuscule(), 
					serviceCivilService.getAdresses(noIndElly, RegDate.get()).principale.getLocalite(), 
					"L'adresse principale n'est pas à " + commune.getNomMinuscule());
		}
	...
	public void check2() throws Exception {

		{
			...
			assertEquals(paysDepart.getNoOFS(),
					serviceCivilService.getAdresses(noIndSven, dateDepart.addDays(1)).principale.getNoOfsPays(),
		...
		
		{
			// vérification que les adresses civiles sont HS
			assertEquals(paysDepart.getNoOFS(),
					serviceCivilService.getAdresses(noIndElly, dateDepart.addDays(1)).principale.getNoOfsPays(),
					"L'adresse principale n'est pas à " + nouvelleCommune.getNomMinuscule());
		}
