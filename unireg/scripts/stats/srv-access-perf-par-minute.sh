#! /bin/bash -
# on va essayer de sortir, par utilisateur méthode le temps moyen de réponse par minute
...
done < <(cat_file "$FILE" | sed -e 's/[^a-zA-Z0-9 :.={}-]\+//g' -e 's/{.*}//' | awk " \$8 ~ /^$SERVICE\$/ { print \$3 \" \" \$4 \" \" \$6; }" | sed -e 's/-/\//g' -e 's/[^0-9\.: /]//g' -e 's/:[0-9]\+\.[0-9]\+//')
