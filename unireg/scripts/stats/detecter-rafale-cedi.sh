...
ANALYSIS=$(read_file "$FILE" | grep "\[cedi\]" | grep "\(QuittancerDeclaration\|AcknowledgeTaxDeclarationsRequest\)" | filter_timestamp $TS_SEUIL | sed -e 's/\(DemandeQuittancementDeclaration\|AcknowledgeTaxDeclarationRequest\)/\n/g' | grep "\(DeclarationImpotOrdinaireKey\|OrdinaryTaxDeclarationKey\)" | sed -e 's/,/ /g' | sed -e 's/[^0-9 ]\+//g' | awk '{ print $1 "-" $2 "-" $3; }' | sort | uniq -c | awk '$1 > 49 { print $0; }') 
