...
echo "UPDATE ${TABLE_PREFIX}TIERS SET TIERS_TYPE='MenageCommun', LOG_MDATE=CURRENT_DATE, LOG_MUSER='SQL-SIFISC-$JIRA', PP_HABITANT=NULL, NUMERO_INDIVIDU=NULL, INDEX_DIRTY=1 WHERE NUMERO=$PP AND TIERS_TYPE='PersonnePhysique';"
echo "DELETE FROM ${TABLE_PREFIX}SITUATION_FAMILLE WHERE CTB_ID=$PP OR TIERS_PRINCIPAL_ID=$PP;"
echo "DELETE FROM ${TABLE_PREFIX}RAPPORT_ENTRE_TIERS WHERE TIERS_SUJET_ID=$PP AND RAPPORT_ENTRE_TIERS_TYPE='AppartenanceMenage';"
echo "DELETE FROM ${TABLE_PREFIX}IDENTIFICATION_PERSONNE WHERE NON_HABITANT_ID=$PP;"
echo
