...
echo "UPDATE TIERS SET ANNULATION_DATE=NULL, ANNULATION_USER=NULL, LOG_MDATE=CURRENT_DATE, LOG_MUSER='SQL-SIFISC-$JIRA', INDEX_DIRTY=1 WHERE NUMERO=$CTB AND ANNULATION_DATE IS NOT NULL;"
echo
echo "-- Réouverture des fors fermés pour annulation au $DATE_ANNULATION"
echo "UPDATE FOR_FISCAL SET LOG_MDATE=CURRENT_DATE, LOG_MUSER='SQL-SIFISC-$JIRA', DATE_FERMETURE=NULL, MOTIF_FERMETURE=NULL WHERE TIERS_ID=$CTB AND ANNULATION_DATE IS NULL AND DATE_FERMETURE=$DATE_ANNULATION AND MOTIF_FERMETURE='ANNULATION';"
