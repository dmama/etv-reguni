...
migregFileOrig=migreg-release.zip
migregFileDest=migreg-release-${env}-${version}-${DATE}.zip
...
(cd unireg/migreg && mvn $MVN_OPTS assembly:assembly)
if [ $? != 0 ]; then
	echo "!!! Erreur lors de l'assembly de migreg"
	exit 1
fi
...
cp unireg/migreg/target/$migregFileOrig unireg/migreg/target/$migregFileDest
...
sudo cp -v unireg/migreg/target/$migregFileDest $release_dir
...
scp unireg/migreg/target/$migregFileDest $user:$REM_DIR
