...
/**
 * Outil pour aller chercher les valeurs "raison sociale", "complément nom", "case postale", "rue" et "localité"
 * des débiteurs de prestation imposable dont les numéros sont fournis
 */
public class AdresseExtractorIS {
...
//	private static final String pwdWebService = "se renseigner...";
	...
	private static final String nomFichier = "debiteurs.csv";
	...
	public static void main(String[] args) throws Exception {
		...
		batchTiers.getParts().add(TiersPart.ADRESSES);

		final InputStream in = AdresseExtractorIS.class.getResourceAsStream(nomFichier);
		...
		for (List<Long> lot : lots) {
			...
			for (BatchTiersEntry entry : result.getEntries()) {
				...
				else if (!(tiers instanceof Debiteur)) {
					System.err.println(String.format("%d n'est pas un débiteur de prestation imposable (%s)", entry.getNumber(), tiers.getClass().getSimpleName()));
				}
				else if (tiers.getAdresseCourrier() == null) {
					System.err.println(String.format("%d n'a pas d'adresse de courier", entry.getNumber()));
				}
				else {
					final Debiteur deb = (Debiteur) tiers;
					final Adresse adr = tiers.getAdresseCourrier();
					System.out.println(String.format("%d;%s;%s;%s;%s;%s", entry.getNumber(),
							StringUtils.trimToEmpty(deb.getRaisonSociale()),
							StringUtils.trimToEmpty(deb.getComplementNom()),
							StringUtils.trimToEmpty(adr.getRue()),
							StringUtils.trimToEmpty(adr.getCasePostale()),
							StringUtils.trimToEmpty(adr.getLocalite())));
				}
			}
		}
	}
