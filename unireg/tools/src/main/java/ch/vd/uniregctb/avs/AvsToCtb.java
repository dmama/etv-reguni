	...
	public static void main(String[] args) throws Exception {
		...
		try {
			for (String avs : avsList) {
				try {
					final SearchResult result = WebServiceV5Helper.searchParty(urlWebService, userWebService, pwdWebService, userId, oid, null, null, avs, null, EnumSet.of(PartySearchType.NATURAL_PERSON));
					if (result.getError() != null) {
						throw new WebServiceException(result.getError().getErrorMessage());
					}
