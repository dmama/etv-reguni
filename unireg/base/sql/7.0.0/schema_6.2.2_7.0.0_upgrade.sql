...
---
--- Gestion de la radiation des immeubles RF
---
ALTER TABLE RF_IMMEUBLE ADD DATE_RADIATION NUMBER(10) NULL;

---
--- Cleanup des tables RF
---

-- ajout de la contrainte unique
DROP INDEX IDX_BATIMENT_MASTER_ID_RF;
CREATE UNIQUE INDEX IDX_BATIMENT_MASTER_ID_RF ON RF_BATIMENT (MASTER_ID_RF);
-- colonne et index inutiles
DROP INDEX IDX_DROIT_ID_RF;
ALTER TABLE RF_DROIT DROP COLUMN ID_RF;
-- changement de prefix pour être cohérent avec les autres indexes
DROP INDEX CS_IMMEUBLE_RF_ID;
CREATE UNIQUE INDEX IDX_IMMEUBLE_RF_ID ON RF_IMMEUBLE (ID_RF);
-- correction du nom de l'index
DROP INDEX IDS_COMMUNE_NO_OFS;
