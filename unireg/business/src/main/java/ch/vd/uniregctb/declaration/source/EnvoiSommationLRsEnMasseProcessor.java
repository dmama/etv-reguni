...
private final Logger LOGGER = Logger.getLogger(EnvoiLRsEnMasseProcessor.class);
	...
	public EnvoiSommationLRsResults run(CategorieImpotSource categorie, final RegDate dateTraitement, StatusManager status) {
		...
		BatchTransactionTemplate<Long> template = new BatchTransactionTemplate<Long>(list, BATCH_SIZE, Behavior.REPRISE_AUTOMATIQUE,
				transactionManager, s, hibernateTemplate);
		template.execute(new BatchCallback<Long>() {

			private List<Long> batchCourant;
			private EnvoiSommationLRsResults rapport;

			@Override
			public void beforeTransaction() {
				rapport = new EnvoiSommationLRsResults(dateTraitement);
			}
