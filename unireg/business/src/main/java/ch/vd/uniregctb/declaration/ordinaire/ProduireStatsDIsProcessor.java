...
public class ProduireStatsDIsProcessor {
	...
	public StatistiquesDIs run(int anneePeriode, RegDate dateTraitement, StatusManager statusManager) throws DeclarationException {

		final StatusManager status = statusManager != null ? statusManager : new LoggingStatusManager(LOGGER);
		final StatistiquesDIs rapport = new StatistiquesDIs(anneePeriode, dateTraitement);
		...
		final BatchTransactionTemplate<Long> template = new BatchTransactionTemplate<Long>(listeComplete, BATCH_SIZE, Behavior.SANS_REPRISE, transactionManager, status, hibernateTemplate);
		template.execute(new BatchCallback<Long>() {

			@Override
			public boolean doInTransaction(final List<Long> batch) throws Exception {
