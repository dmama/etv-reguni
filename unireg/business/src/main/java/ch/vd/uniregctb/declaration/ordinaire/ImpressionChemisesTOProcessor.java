...
public class ImpressionChemisesTOProcessor {
	...
	public ImpressionChemisesTOResults run(int nombreMax, final Integer noColOid, StatusManager status) {
		...
		final BatchTransactionTemplate<Long> batchTemplate = new BatchTransactionTemplate<Long>(ids, BATCH_SIZE, Behavior.REPRISE_AUTOMATIQUE, transactionManager, status, hibernateTemplate);
		batchTemplate.execute(new BatchCallback<Long>() {
			...
			public void beforeTransaction() {
				...
				results = new ImpressionChemisesTOResults(nbMaxTO, nomOid);
			}
			...
			public void afterTransactionRollback(Exception e, boolean willRetry) {
				...
				if (willRetry) {
					...
					resultsFinaux.addOnCommitException(id, e);
					results = null;
				}
			}
		...
		if (s.interrupted()) {
			s
					.setMessage("L'impression des chemises de taxation d'office a été interrompue. Nombre de chemises imprimées au moment de l'interruption = "
