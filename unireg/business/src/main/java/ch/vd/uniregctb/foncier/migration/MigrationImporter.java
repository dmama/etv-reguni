...
public abstract class MigrationImporter {
	...
	@NotNull
	protected ImmeubleRF determinerImmeuble(MigrationDDKey key, Map<String, Commune> mapCommunes, DonneesFusionsCommunes fusionData) throws ObjectNotFoundException {
	...
	private ImmeubleRF findImmeuble(Commune commune, MigrationParcelle parcelle, Map<String, Commune> mapCommunes) {
		...
		if (commune.isFraction()) {
			...
			if (communeFaitiere != null) {
				final ImmeubleRF immeubleSurFaitiere = immeubleRFDAO.findImmeubleActif(communeFaitiere.getNoOFS(), parcelle.getNoParcelle(), parcelle.getIndex1(), parcelle.getIndex2(), parcelle.getIndex3(), FlushMode.MANUAL);;
				if (immeubleSurFaitiere != null) {
					return immeubleSurFaitiere;
				}
			}
		}
