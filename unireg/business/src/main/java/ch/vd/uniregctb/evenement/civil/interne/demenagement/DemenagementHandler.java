...
public class DemenagementHandler extends EvenementCivilHandlerBase {
	...
	protected void validateSpecific(EvenementCivilInterne target, List<EvenementCivilExterneErreur> errors, List<EvenementCivilExterneErreur> warnings) {
		...
		if (ancienneAdresse == null || ancienneAdresse.getDateFin() == null) {
			errors.add(new EvenementCivilExterneErreur("L'individu n°" + demenagement.getNoIndividu() + " n'a jamais déménagé"));
		}
		else if (!demenagement.getDate().isAfter(ancienneAdresse.getDateFin())) {
			errors.add(new EvenementCivilExterneErreur("La date du déménagement est antérieure à la date de fin de l'ancienne adresse"));
		}
		...
		if (nouvelleCommune == null || !nouvelleCommune.isVaudoise()) {
			errors.add(new EvenementCivilExterneErreur("La nouvelle adresse est en dehors du canton"));
		}
