...
public class ProduireRolesCommuneProcessor {
	...
	private ProduireRolesResults doRun(final int anneePeriode, final StatusManager status, final VarianteProductionRole variante) {
		...
		final BatchTransactionTemplate<Long> template = new BatchTransactionTemplate<Long>(list, BATCH_SIZE, BatchTransactionTemplate.Behavior.REPRISE_AUTOMATIQUE, transactionManager, status, hibernateTemplate);
		template.setReadonly(true);
		template.execute(new BatchTransactionTemplate.BatchCallback<Long>() {

			private ProduireRolesResults rapport;
			private Long ctbId;
...
// TODO jde/msn implémenter la méthode dans le service civil service getIndividus
//				preloadIndividus(tierz, anneePeriode);

				// deuxième boucle pour le traitement proprement dit
				for (Tiers tiers : tierz) {

					ctbId = tiers.getNumero();
					ctb = (Contribuable) tiers;
			...
			public void beforeTransaction() {
				...
				rapport = variante.creerRapport(anneePeriode, today, false);
				ctbId = null;
				ctb = null;
			}
			...
			public void afterTransactionRollback(Exception e, boolean willRetry) {
				...
				if (willRetry) {
					...
					rapport.addErrorException(ctbId, e);
				}
			}
		});
