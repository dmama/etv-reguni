...
public class ReinitialiserBaremeDoubleGainProcessor {
	...
	public ReinitialiserBaremeDoubleGainResults run(final RegDate dateTraitement, StatusManager s) {
		...
		template.execute(new BatchCallback<Long>() {
			...
			private Long idSituation = null;

			@Override
			public void beforeTransaction() {
				rapport = new ReinitialiserBaremeDoubleGainResults(dateTraitement);
				idSituation = null;
			}
			...
			public void afterTransactionRollback(Exception e, boolean willRetry) {
				if (willRetry) {
					...
					rapportFinal.addErrorException(idSituation, e);
					rapport = null;
				}
			}
