...
import ch.vd.unireg.xml.party.corporation.v2.SogcEdition;
...
public class CorporationStrategy extends TaxPayerStrategy<Corporation> {
	...
	private void fillBankAccountFromCorporationRepresentative(BankAccount cb, long noPM, ServicePersonneMoraleService servicePM) {
		...
		if (pm != null) {
			...
			if (cpm != null && !cpm.isEmpty()) {
				...
				cb.setFormat(EnumHelper.coreToXMLv2(c.getFormat()));
	...
	private static List<BankAccount> account2web(long numeroEntreprise, List<CompteBancaire> comptes) {
		...
		for (CompteBancaire c : comptes) {
			...
			compte.setFormat(EnumHelper.coreToXMLv2(c.getFormat()));
	...
	private TaxResidence mainTaxResidence2web(ch.vd.uniregctb.interfaces.model.ForPM f, ServiceInfrastructureService serviceInfra) {
		...
		ffp.setTaxationAuthorityType(host2web(f.getTypeAutoriteFiscale(), f.getNoOfsAutoriteFiscale(), serviceInfra,f.getDateFin()));
	...
	private TaxationAuthorityType host2web(TypeNoOfs type, int noOfs, ServiceInfrastructureService serviceInfra,RegDate dateTo) {
		switch (type) {
		case COMMUNE_CH:
			final Commune commune;
			try {
				commune = serviceInfra.getCommuneByNumeroOfs(noOfs, dateTo);
			}
			catch (ServiceInfrastructureException e) {
