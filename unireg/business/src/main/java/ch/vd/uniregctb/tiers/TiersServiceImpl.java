...
public class TiersServiceImpl implements TiersService {
	...
	public PersonnePhysique changeHabitantenNH(PersonnePhysique habitant) {
		...
		Iterator<Permis> itPermis = colPermis.iterator();
		while (itPermis.hasNext()) {
			Permis permis = itPermis.next();
		...
		for (VueSituationFamille vueSF : histoSF) {
			...
			hibernateTemplate.execute(new HibernateCallback() {
				...
				public Object doInHibernate(Session session) throws HibernateException, SQLException {
					...
					SQLQuery query = session.createSQLQuery(updateSFQuery);
	 ...
	 *             si la nationalite ne peut être déterminée
	 */
	// TODO(MSI) : ajouter un test dans TiersServiceTest par rapport a l'origine (voir Cobertura)
	...
	public boolean isAvecPermisC(Individu individu, RegDate date) {
		...
		for (Object obj : permiss) {
			 ...
			 * donc permis.getDateFinValidite().isAfter(date) devient permis.getDateFinValidite().isAfterOrEqual(date)
			 *
	...
	public boolean isInMenageCommun(PersonnePhysique personne, RegDate date) {
		return (findMenageCommun(personne, date) != null) ? true : false;
	}
	...
	public Individu getIndividu(PersonnePhysique personne) {
		...
		if(personne.getNumeroIndividu() != null && personne.getNumeroIndividu() != 0) {
			...
			if (individu == null) {
				...
				if (noIndividu != null) {
					individu = getServiceCivilService().getIndividu(noIndividu, 2400);
	...
	public List<PersonnePhysique> getSourciers(int noSourcier) {
		return tiersDAO.getSourciers(noSourcier);
	}

	public List<PersonnePhysique> getAllMigratedSourciers() {
		return tiersDAO.getAllMigratedSourciers();
	}
	...
	public ForFiscalAutreImpot openForAutreImpot(Tiers tiers, RegDate dateOuverture, RegDate dateFermeutre, int noOfs, TypeAutoriteFiscale typeAutoriteFiscale, GenreImpot genreImpot) {

		ForFiscalAutreImpot autre = new ForFiscalAutreImpot();
		autre.setGenreImpot(genreImpot);
		autre.setDateDebut(dateOuverture);
		autre.setNumeroOfsAutoriteFiscale(noOfs);
		autre.setTypeAutoriteFiscale(typeAutoriteFiscale);
		autre = (ForFiscalAutreImpot) addAndSave(tiers, autre);

		if (tiers.validate().errorsCount() == 0) {
			afterForAutreImportAdded(tiers, autre);
		}
	...
	public ForFiscalSecondaire closeForFiscalSecondaire(Contribuable contribuable, ForFiscalSecondaire forFiscalSecondaire,
		...
		if (forFiscalSecondaire != null) {
			...
			if (!contribuable.getForFiscalPrincipalAt(forFiscalSecondaire.getDateFin()).getTypeAutoriteFiscale().equals(TypeAutoriteFiscale.COMMUNE_OU_FRACTION_VD)) {
				boolean isLast = true;
	 ...
	 *
	 * @param contribuable
	 * @param dateChangementModeImposition
	 * @param modeImposition
	 * @param motifFor
	 * @return
	 */
	...
	public void fusionne(PersonnePhysique habitant, PersonnePhysique nonHabitant) {
		...
		Iterator<ForFiscal> itFors = nonHabitant.getForsFiscaux().iterator();
		while (itFors.hasNext()) {
			ForFiscal forFiscalSource = itFors.next();
...
//		}
	...
	public List<Long> getHabitantsForMajorite(RegDate dateReference) {
		return tiersDAO.getHabitantsForMajorite(dateReference);
	}

	public DebiteurPrestationImposable getDebiteurPrestationImposable(long numeroDPI){
		return tiersDAO.getDebiteurPrestationImposableByNumero(numeroDPI);
	}
	...
	public String getRoleAssujettissement(Tiers tiers, RegDate date) {
		...
		if (tiers instanceof PersonnePhysique
				&& ((PersonnePhysique) tiers).isDebiteurInactif()
	 ...
	 * {@inheritDoc}
	 */
	public CorrectionFlagHabitantSurPersonnesPhysiquesResults corrigeFlagHabitantSurPersonnesPhysiques(StatusManager statusManager) {
		...
		final CorrectionFlagHabitantSurPersonnesPhysiquesResults rapport = processor.corrigeFlagSurPersonnesPhysiques();
		return rapport;
	}
	 ...
	 * {@inheritDoc}
	 */
	public CorrectionFlagHabitantSurMenagesResults corrigeFlagHabitantSurMenagesCommuns(StatusManager statusManager) {
		final CorrectionFlagHabitantProcessor processor = new CorrectionFlagHabitantProcessor(hibernateTemplate, this, transactionManager, statusManager);
		final CorrectionFlagHabitantSurMenagesResults rapport = processor.corrigeFlagSurMenages();
		return rapport;
	}
