...
public class EvenementFiscalV2SenderImpl implements EvenementFiscalSender, InitializingBean {
	...
	private interface OutputDataFactory<I extends EvenementFiscal, O extends ch.vd.unireg.xml.event.fiscal.v2.EvenementFiscal> {
		@NotNull
		O build(@NotNull I evenementFiscal);
	}
	...
	private static Map<Class<? extends EvenementFiscal>, OutputDataFactory<? extends EvenementFiscal, ? extends ch.vd.unireg.xml.event.fiscal.v2.EvenementFiscal>> buildOutputDataFactories() {
		...
		registerOutputDataFactory(map, EvenementFiscalDeclaration.class, new DeclarationFactory());
	...
	private static class DeclarationFactory implements OutputDataFactory<EvenementFiscalDeclaration, ch.vd.unireg.xml.event.fiscal.v2.EvenementFiscalDeclaration> {
		@NotNull
		@Override
		public ch.vd.unireg.xml.event.fiscal.v2.EvenementFiscalDeclaration build(@NotNull EvenementFiscalDeclaration evenementFiscal) {
	...
	protected static TypeEvenementFiscalDeclaration mapType(EvenementFiscalDeclaration.TypeAction type) {
		switch (type) {
		case ANNULATION:
	...
	private static class ForFactory implements OutputDataFactory<EvenementFiscalFor, ch.vd.unireg.xml.event.fiscal.v2.EvenementFiscalFor> {
		...
		public ch.vd.unireg.xml.event.fiscal.v2.EvenementFiscalFor build(@NotNull EvenementFiscalFor evenementFiscal) {
			...
			instance.setLocalisationFor(EnumHelper.coreToXMLv2(forFiscal.getTypeAutoriteFiscale()));
			if (forFiscal instanceof ForFiscalAvecMotifs) {
				...
				if (instance instanceof OuvertureFor) {
					((OuvertureFor) instance).setMotifOuverture(EnumHelper.coreToXMLv2(avecMotifs.getMotifOuverture()));
				}
