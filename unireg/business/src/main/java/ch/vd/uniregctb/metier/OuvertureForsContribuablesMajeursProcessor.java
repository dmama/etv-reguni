...
public class OuvertureForsContribuablesMajeursProcessor {
	...
	public OuvertureForsResults run(final RegDate dateReference, StatusManager status) {
		...
		final BatchTransactionTemplate<Long> template = new BatchTransactionTemplate<Long>(list, BATCH_SIZE, Behavior.REPRISE_AUTOMATIQUE,
		...
		template.execute(new BatchCallback<Long>() {
			...
			public void beforeTransaction() {
				rapport = new OuvertureForsResults(dateReference);
			}
	...
	private void fillInfoDomicile(PersonnePhysique habitant, final HabitantData data, RegDate dateReference) throws OuvertureForsException {
		...
		try {
			adresseDomicile = adresseService.getAdresseFiscale(habitant, TypeAdresseTiers.DOMICILE, dateReference);
		}
