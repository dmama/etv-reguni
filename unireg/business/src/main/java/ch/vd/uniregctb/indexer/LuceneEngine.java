...
public abstract class LuceneEngine {
	...
	public static void debugHits(Hits hits) {
		...
		if (false) {
			...
			try {
				...
				while (iter.hasNext()) {
					Hit hit = iter.next();
					String id = hit.get(F_DOCID);
					String type = hit.get(F_DOCTYPE);
					String entityId = hit.get(F_ENTITYID);
					String nom = hit.get("hostindividuNOM");
