...
public class RapportServiceImpl implements RapportService, ApplicationContextAware {
	...
	@Override
	public DesappariementREERapport generateRapport(DesappariementREEResults results, StatusManager s) {
		final StatusManager status = (s == null ? new LoggingStatusManager(LOGGER) : s);

		final String nom = "RapportDesappariementEtablissementsREE";
		final String description = "Rapport d'exécution du job de désappariement des établissements REE.";
		final Date dateGeneration = DateHelper.getCurrentDate();

		try {
			return docService.newDoc(DesappariementREERapport.class, nom, description, "pdf", new DocumentService.WriteDocCallback<DesappariementREERapport>() {
				@Override
				public void writeDoc(DesappariementREERapport doc, OutputStream os) throws Exception {
					final PdfDesappariementREERapport document = new PdfDesappariementREERapport();
					document.write(results, nom, description, dateGeneration, os, status);
				}
			});
		}
