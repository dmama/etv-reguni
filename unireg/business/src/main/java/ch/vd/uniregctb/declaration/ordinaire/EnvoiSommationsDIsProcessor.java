...
public class EnvoiSommationsDIsProcessor  {
	...
	public EnvoiSommationsDIsResults run(final RegDate dateTraitement, final boolean miseSousPliImpossible, final Integer nombreMax, StatusManager statusManager) {
		...
		final BatchTransactionTemplate<Long> t = new BatchTransactionTemplate<Long>(dis, BATCH_SIZE, Behavior.REPRISE_AUTOMATIQUE, transactionManager, status, hibernateTemplate);
		t.execute(new BatchCallback<Long>() {
			...
			public void beforeTransaction() {
				rapport = new EnvoiSommationsDIsResults();
			}
			...
			public boolean doInTransaction(List<Long> batch) {
				...
				try {
					...
					while (iter.hasNext() && ! status.interrupted() && ( nombreMax == 0 || (rapportFinal.getTotalDisSommees()  + rapport.getTotalDisSommees() )< nombreMax) ) {
						...
						if (finDelai.isBefore(dateTraitement)) {
							try {
								...
								if (assujettissements == null || assujettissements.isEmpty()) {
									...
									LOGGER.info(msg);
									rapport.addNonAssujettissement(di);
								} else {
									...
									if (isIndigent(di,	assujettissements)) {
										...
										LOGGER.info(
												String.format(
														...
														di.getDateDebut(),
														di.getDateFin()));
										rapport.addDiSommee(di.getPeriode().getAnnee(), di);
									}
								}
							}
							catch (RuntimeException e) {
								rapport.addError(di,e.getMessage());
				...
				return  (nombreMax == 0 || (rapportFinal.getTotalDisSommees()  + rapport.getTotalDisSommees() ) < nombreMax) && !status.interrupted();
			}
