...
public abstract class MockServiceCivil implements ServiceCivilService {
	...
	/**
	 * Map des adresses, indexée par numéro d'individu.
	 */
	private final Map<Long, List<Adresse>> adressesMap = new HashMap<Long, List<Adresse>>();
	...
	public static Adresse newAdresse(EnumTypeAdresse type, Rue rue, String casePostale, Localite localite, RegDate debutValidite, RegDate finValidite) {
		...
		Assert.isTrue(rue == null || rue.getNoLocalite() == localite.getNoOrdre(), "La rue et la localité ne correspondent pas");
	...
	protected void add(Individu individu, Adresse adresse) {
		...
		List<Adresse> list = adressesMap.get(numero);
		if (list == null) {
			list = new ArrayList<Adresse>();
			adressesMap.put(numero, list);
		}
		list.add(adresse);
	}
	...
	public AdressesCiviles getAdresses(long noIndividu, RegDate date) {
	...
	public AdressesCivilesHisto getAdressesHisto(long noIndividu) {
	...
	public Collection<Adresse> getAdressesActives(Long numeroIndividu, RegDate date) {
		...
		final List<Adresse> adresses = adressesMap.get(numeroIndividu);
		if (adresses != null) {
			if (date == null) {
