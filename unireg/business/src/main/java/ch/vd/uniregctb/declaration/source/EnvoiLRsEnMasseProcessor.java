...
public class EnvoiLRsEnMasseProcessor {
	...
	public EnvoiLRsResults run(final RegDate dateFinPeriode, StatusManager status) {
		...
		final BatchTransactionTemplate<Long> template = new BatchTransactionTemplate<Long>(list, BATCH_SIZE, Behavior.REPRISE_AUTOMATIQUE, transactionManager, s, hibernateTemplate);
		template.execute(new BatchCallback<Long>() {
			...
			public void beforeTransaction() {
				rapport = new EnvoiLRsResults(dateTraitement, dateFinPeriode);
			}

			@Override
			public boolean doInTransaction(List<Long> batch) throws Exception {
				batchCourant = batch;
				traiteBatch(batch, dateFinPeriode, s, rapport);
