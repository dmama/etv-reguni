...
public class ServiceSecuriteHostInterfaces implements ServiceSecuriteService {
	 ...
	 */
	@Override
	public List<ch.vd.uniregctb.interfaces.service.host.Operateur> getUtilisateurs(List<TypeCollectivite> typesCollectivite) {
		try {
			...
			List<ch.vd.uniregctb.interfaces.service.host.Operateur> lOperateurs = new ArrayList<>();
			for (int j = 0; j < operateurs.length; j++) {
				lOperateurs.add(ch.vd.uniregctb.interfaces.service.host.Operateur.get(operateurs[j]));
			}
			return lOperateurs;
	 ...
	 */
	@Override
	public ch.vd.uniregctb.interfaces.service.host.Operateur getOperateur(long individuNoTechnique) {
		try {
			// [SIFISC-7231] On ne veut pas se limiter aux opérateurs actuellement valides
			return ch.vd.uniregctb.interfaces.service.host.Operateur.get(serviceSecurite.getOperateurTous(individuNoTechnique));
		}
		catch (Exception e) {
	 ...
	 */
	@Override
	public ch.vd.uniregctb.interfaces.service.host.Operateur getOperateur(String visa) {
		try {
			// [SIFISC-7231] On ne veut pas se limiter aux opérateurs actuellement valides
			return ch.vd.uniregctb.interfaces.service.host.Operateur.get(serviceSecurite.getOperateur(visa));
		}
		catch (Exception e) {
