...
public class PdfListeNoteRapport extends PdfRapport {
		...
		// adresses resolues
			{
				...
				final String contenu = getCsvContribuablesNotes(results.listeContribuableAvecNote, filename, status);
	...
	private <T extends ListeNoteResults.InfoContribuableWithNote> String getCsvContribuablesNotes(List<T> liste, String filename, StatusManager status) {
		String contenu = null;
		if (liste != null && liste.size() > 0) {

			final StringBuilder b = new StringBuilder(liste.size() * 100);
			b.append("NUMERO CTB").append(COMMA).append("LOCALITE\n");
