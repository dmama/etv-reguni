...
public class QueryConstructor {
	...
	private void addNomRaison(BooleanQuery fullQuery) throws IndexerException {
		...
		if (!StringUtils.isEmpty(criteria.getNomRaison())) {
			...
			case CONTIENT:
			default:
				final Query subQuery = LuceneEngine.getTermsContient(TiersSearchFields.NOM_RAISON, nomCourrier, tokenMinLength);
				if (subQuery != null) {
					query.add(subQuery, should);
				}
				final Query subQuery2 = LuceneEngine.getTermsContient(TiersSearchFields.AUTRES_NOM, nomCourrier, tokenMinLength);
				if (subQuery2 != null) {
	...
	private void addDateNaissance(BooleanQuery fullQuery) throws IndexerException {
		...
		if (criteria.getDateNaissance() != null) {
			final Query q = LuceneEngine.getTermsCommence(TiersSearchFields.DATE_NAISSANCE, RegDateHelper.toIndexString(criteria.getDateNaissance()), 0);
			fullQuery.add(q, must);
		}
	}
