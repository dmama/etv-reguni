 ...
 */
public class ProduireRolesResults extends JobResults {
	...
	public static class InfoCommune {
		...
		/**
		 * Merge les données de la structure donnée dans la mienne
		 */
		public void merge(InfoCommune src) {
			Assert.isEqual(noOfs, src.noOfs);
			for (Map.Entry<Long, InfoContribuable> entry : src.infosContribuables.entrySet()) {
				final Long noCtb = entry.getKey();
				if (infosContribuables.containsKey(noCtb)) {
					Assert.fail("Contribuable " + noCtb + " vu plusieurs fois ?");
				}
				else {
					infosContribuables.put(noCtb, entry.getValue());
				}
			}
		}
	}
	...
	public static class InfoContribuable {
		...
		public InfoContribuable(Contribuable ctb, int annee, AdresseService adresseService, TiersService tiersService) {
			...
			try {
				adresseEnvoi = adresseService.getAdresseEnvoi(ctb, RegDate.get(annee, 12, 31));
			}
	...
	public void addErrorException(Long ctbId, Exception e) {
		ctbsEnErrors.add(new Erreur(ctbId, null, ErreurType.EXCEPTION, e.getMessage()));
	}
	...
	public void addAll(ProduireRolesResults rapport) {
		...
		ctbsTraites += rapport.ctbsTraites;

		// merge des infos communes
		for (Map.Entry<Integer, InfoCommune> entry : rapport.infosCommunes.entrySet()) {
			final Integer noOfsCommune = entry.getKey();
			if (infosCommunes.containsKey(noOfsCommune)) {
				final InfoCommune info = infosCommunes.get(noOfsCommune);
				info.merge(entry.getValue());
			}
			else {
				infosCommunes.put(noOfsCommune, entry.getValue());
			}
		}
	}
}
