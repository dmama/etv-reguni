...
public class ImpressionDeclarationImpotOrdinaireHelperImpl implements ImpressionDeclarationImpotOrdinaireHelper  {
	...
	protected InfoDocument remplitInfoDocument(Declaration declaration) throws EditiqueException {
		...
		try {
			AdresseEnvoiDetaillee adresseEnvoiDetaillee = adresseService.getAdresseEnvoi(declaration.getTiers(), null);
			String idEnvoi = new String("");
	 ...
	 */
	protected InfoEnteteDocument remplitEnteteDocument(Declaration declaration) throws AdressesResolutionException, InfrastructureException {
	 ...
	 */
	private Expediteur remplitExpediteur(Declaration declaration, InfoEnteteDocument infoEnteteDocument) throws AdressesResolutionException, InfrastructureException {
		//
		// Expediteur
		//
		// [UNIREG-1257] tenir compte de l'OID valide durant la période de validité de la déclaration
	...
	private void remplitAdresseRetour(DeclarationImpotOrdinaire declaration, DIRetour di) throws EditiqueException {
		...
		if (col.getNumeroCollectiviteAdministrative() == ServiceInfrastructureService.noCEDI) { // Cas spécial pour le CEDI
			...
			try {
				adresse = adresseService.getAdresseEnvoi(col, null);
			}
	...
	private void remplitAdresseSuite(DeclarationImpotOrdinaire declaration, DI di) throws EditiqueException {
		...
		try {
			adresseEnvoi = adresseService.getAdresseEnvoi(declaration.getTiers(), null);
		}
		catch (AdressesResolutionException e) {
			throw new EditiqueException(e);
		}
	...
	private String calculIndividuNomPrenom(DeclarationImpotOrdinaire declaration, PersonnePhysique pp) throws EditiqueException {
		...
		try {
			adresseEnvoi = adresseService.getAdresseEnvoi(pp, declaration.getDateFin());
		}
		catch (AdressesResolutionException e) {
			throw new EditiqueException(e);
		}
		String indnomprenom1 = adresseEnvoi.getLigne1() + " " + adresseEnvoi.getLigne2();
		return indnomprenom1;
	}
	...
	private void remplitAnnexes(DIBase di, List<ModeleFeuilleDocumentEditique> annexes) {
		...
		Iterator<ModeleFeuilleDocumentEditique> itAnnexes = annexes.iterator();
		while (itAnnexes.hasNext()) {
			ModeleFeuilleDocumentEditique annexe = itAnnexes.next();
