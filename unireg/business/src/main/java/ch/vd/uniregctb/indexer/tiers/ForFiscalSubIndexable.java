	...
	private String getForCommuneAsString(ForFiscal forF) throws IndexerException {
		...
		try {
			...
			if (typeForFiscal == TypeAutoriteFiscale.COMMUNE_OU_FRACTION_VD) {
					...
					if (com == null) {
						throw new DonneesCivilesException(Arrays.asList("Commune pas trouvée: noOfsEtendu=" + forF.getNumeroOfsAutoriteFiscale()));
					}
			...
			// Commune suisse
					...
					Commune com = infraService.getCommuneByNumeroOfsEtendu(forF.getNumeroOfsAutoriteFiscale());
					if (com == null) {
						throw new DonneesCivilesException(Arrays.asList("Commune pas trouvée: noOfs=" + forF.getNumeroOfsAutoriteFiscale()));
					}
					forStr = com.getNomMinuscule();
			}
			// Pays
					...
					if (p == null) {
						throw new DonneesCivilesException(Arrays.asList("Pays pas trouvé: noOfs=" + forF.getNumeroOfsAutoriteFiscale()));
					}
