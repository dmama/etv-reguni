...
public class DeterminerMouvementsDossiersEnMasseProcessor {
	...
	public DeterminerMouvementsDossiersEnMasseResults run(final RegDate dateTraitement, StatusManager s) {
		...
		final BatchTransactionTemplate<Long> template = new BatchTransactionTemplate<Long>(ctbs, BATCH_SIZE, BatchTransactionTemplate.Behavior.REPRISE_AUTOMATIQUE, transactionManager, status, hibernateTemplate);
		template.execute(new BatchTransactionTemplate.BatchCallback<Long>() {

			private Long idCtb;
			private DeterminerMouvementsDossiersEnMasseResults rapport;

			@Override
			public void beforeTransaction() {
				rapport = new DeterminerMouvementsDossiersEnMasseResults(dateTraitement);
				idCtb = null;
			}
