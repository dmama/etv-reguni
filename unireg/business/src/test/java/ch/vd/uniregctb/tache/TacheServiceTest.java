	...
	public void testGenereDecesDepuisOuvertureForPrincipal() throws Exception {
		...
		doInNewTransaction(new TxCallback() {
			...
			public Object execute(TransactionStatus status) throws Exception {
				...
				Iterator<RapportEntreTiers> iter = rapport.iterator();
				while (iter.hasNext()) {
					iter.next().setDateFin(RegDate.get(2006, 6, 11));
				}
	...
	public void testGenerateTacheDivorcePeriodePassee() throws Exception {
		...
		doInNewTransaction(new TxCallback() {
			...
			public Object execute(TransactionStatus status) throws Exception {
				...
				EnsembleTiersCouple ensemble = createEnsembleTiersCouple(monsieur, madame, dateMariage);
				MenageCommun menage = ensemble.getMenage();
