...
public class InitialisationIFoncProcessorTest extends BusinessTest {
	...
	protected void runOnSetUp() throws Exception {
		...
		processor = new InitialisationIFoncProcessor(transactionManager, hibernateTemplate, rapprochementRFDAO);
	}
	...
	public void testSimple() throws Exception {
		...
		final Ids ids = doInNewTransactionAndSession(status -> {
			...
			addDroitPersonnePhysiqueRF(null, date(2016, 5, 2), null, "Achat", null, "3458wgfs", new IdentifiantAffaireRF(213, "5823g"), new Fraction(1, 5), GenrePropriete.COPROPRIETE, proprioRF, immeuble, null);
			...
			addDroitHabitationRF(null, date(2017, 3, 1), null, "Un motif, quoi...", null, "5378tgzufbs", null, null, habitantRF, immeuble);

			final PersonnePhysiqueRF usufruitier = addPersonnePhysiqueRF("236gzbfahécf", "Gérard", "Menfais", date(2000, 3, 1));
			addUsufruitRF(null, date(2015, 6, 1), null, "Succession", null, "58gfhfba", null, null, usufruitier, immeuble);
		...
		// extraction au 01.01.2017 (tous sauf le droit d'habitation)
		{
			...
			Assert.assertEquals(2, results.getDroits().size());
			Assert.assertEquals(0, results.getErreurs().size());

			{
				final InitialisationIFoncResults.InfoDroit info = results.getDroits().get(0);
				...
				Assert.assertEquals((Integer) 4514, info.noParcelle);
				Assert.assertEquals((Integer) 4, info.index1);
				Assert.assertEquals((Integer) 2, info.index2);
				Assert.assertEquals((Integer) 1, info.index3);
				Assert.assertEquals("Echallens", info.nomCommune);
				Assert.assertEquals((Integer) MockCommune.Echallens.getNoOFS(), info.noOfsCommune);
			...
			{
				final InitialisationIFoncResults.InfoDroit info = results.getDroits().get(1);
				Assert.assertNotNull(info);
		...
		// extraction au 01.01.2018 (tous)
		{
			...
			Assert.assertNotNull(results);
			Assert.assertEquals(1, results.getNbImmeublesInspectes());
			Assert.assertEquals(3, results.getDroits().size());
			Assert.assertEquals(0, results.getErreurs().size());
			...
			{
				final InitialisationIFoncResults.InfoDroit info = results.getDroits().get(2);
				Assert.assertNotNull(info);
				Assert.assertNull(info.idContribuable);
				Assert.assertNull(info.idCommunaute);
				Assert.assertNotNull(info.identificationRF);
	...
	public void testAvecCommunaute() throws Exception {
		...
		final Ids ids = doInNewTransactionAndSession(status -> {
			...
			addDroitPersonnePhysiqueRF(null, date(2016, 5, 2), null, "Achat", null, "3458wgfs", new IdentifiantAffaireRF(213, "5823g"), new Fraction(1, 5), GenrePropriete.COMMUNE, communisteRF1, immeuble, communaute);
			...
			addDroitPersonnePhysiqueRF(null, date(2017, 3, 1), null, "Un motif, quoi...", null, "5378tgzufbs", new IdentifiantAffaireRF(213, "5823g"), new Fraction(3, 8), GenrePropriete.COMMUNE, communisteRF2, immeuble, communaute);

			addDroitCommunauteRF(null, date(2016, 1, 1), null, "Succession", null, "478tgsbFB", new IdentifiantAffaireRF(213, "5823g"), new Fraction(12, 50), GenrePropriete.INDIVIDUELLE, communaute, immeuble);
