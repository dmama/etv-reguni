...
public class EvenementCivilListenerTest extends BusinessTest {
	...
	public void testOnMessageBad() throws Exception {
		...
		final int id = 54;
		final String xmlContent = createMessage(id, 1000, 9876543, RegDate.get(2007, 9, 18), MockCommune.Lausanne.getNoOFS()); // no individu inconnu
		...
		assertEquals("Le numéro OFS n'a pas été récupéré correctement", new Integer(MockCommune.Lausanne.getNoOFS()), evenement.getNumeroOfsCommuneAnnonce());
	...
	public void testOnMessageDuplicateId() throws Exception {
		...
		final int id = 54;
		final String xmlContent = createMessage(id, TypeEvenementCivil.ARRIVEE_PRINCIPALE_VAUDOISE.getId(), 123, RegDate.get(1980, 1, 1), MockCommune.Lausanne.getNoOFS());
