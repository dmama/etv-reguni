...
public class DepartHandlerTest extends AbstractEvenementHandlerTest {
	...
	private MockDepart createValidDepart(int noIndividu, RegDate dateEvenement, boolean principale) throws Exception {
		...
		// Adresse actuelle
		AdressesCiviles adresseVaud = serviceCivil.getAdresses(noIndividu, dateEvenement);
		...
		// Nouvelles adresses
		AdressesCiviles adresseHorsVaud = serviceCivil.getAdresses(noIndividu, dateEvenement.getOneDayAfter());
	...
	private boolean findMessage(List<EvenementCivilErreur> erreurs, String message) {
		...
		Iterator<EvenementCivilErreur> iteEvenement = erreurs.iterator();
		while (iteEvenement.hasNext()) {
			EvenementCivilErreur evenementErreur = iteEvenement.next();
