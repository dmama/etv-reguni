...
public class LoadAveragerTest extends WithoutSpringTest {
	...
	public void testAverage() throws Exception {
		...
		// à vide
		Assert.assertEquals(0.0, averager.getLoadAverage(), 1e-12);
		...
		for (int i = 0 ; i < nbPoints ; ++ i) {
			...
			Assert.assertEquals("Après l'ajout de 0 à " + i, expected, averager.getLoadAverage(), 1e-12);
		}
		...
		for (int i = nbPoints ; i < 2 * nbPoints ; ++ i) {
			...
			Assert.assertEquals("Après l'ajout de " + i, expected, averager.getLoadAverage(), 1e-12);
		}
		...
		Assert.assertEquals(20, averager.getLoadAverage(), 1e-12);
	}
}
