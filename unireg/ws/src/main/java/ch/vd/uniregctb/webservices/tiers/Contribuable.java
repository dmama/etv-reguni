...
@XmlType(name = "Contribuable", propOrder = {
		"situationFamille", "assujettissement", "periodeImposition"
})
public abstract class Contribuable extends Tiers {
	...
	/**
	 * L'assujettissement à la date demandée, ou <b>null</b> si le tiers n'est pas assujetti.
	 * <p>
	 ...
	 */
	@XmlElement(required = false)
	public Assujettissement assujettissement = null;
	...
	public Contribuable(ch.vd.uniregctb.tiers.Contribuable contribuable, Set<TiersPart> parts, ch.vd.registre.base.date.RegDate date,
		...
		if (parts != null && parts.contains(TiersPart.ASSUJETTISSEMENTS)) {
			...
			final List<ch.vd.uniregctb.metier.assujettissement.Assujettissement> list = ch.vd.uniregctb.metier.assujettissement.Assujettissement
					.determine(contribuable, null, true);
			if (list != null) {
				for (ch.vd.uniregctb.metier.assujettissement.Assujettissement a : list) {
					if (a.isValidAt(date)) {
						this.assujettissement = new Assujettissement(a);
						// [UNIREG-1517] l'assujettissement courant est laissé ouvert
						if (this.assujettissement.dateFin != null) {
							final RegDate aujourdhui = RegDate.get();
							final RegDate dateFin = RegDate.get(this.assujettissement.dateFin.asJavaDate());
							if (dateFin.isAfter(aujourdhui)) {
								this.assujettissement.dateFin = null;
							}
						}
						break;
					}
	...
	private final void copyParts(Contribuable contribuable, Set<TiersPart> parts) {
		if (parts != null && parts.contains(TiersPart.SITUATIONS_FAMILLE)) {
			this.situationFamille = contribuable.situationFamille;
		}

		if (parts != null && parts.contains(TiersPart.ASSUJETTISSEMENTS)) {
			this.assujettissement = contribuable.assujettissement;
		}
