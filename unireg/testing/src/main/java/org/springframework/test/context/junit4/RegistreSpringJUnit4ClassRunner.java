...
import org.springframework.test.context.RegistreTestContextManager;
 ...
 */
public class RegistreSpringJUnit4ClassRunner extends JUnit4ClassRunner {

	private static final Log logger = LogFactory.getLog(RegistreSpringJUnit4ClassRunner.class);

	private final RegistreTestContextManager testContextManager;
	 ...
	 */
	public RegistreSpringJUnit4ClassRunner(Class<?> clazz) throws InitializationError {
		super(clazz);
	 ...
	 */
	protected RegistreTestContextManager createTestContextManager(Class<?> clazz) {
		return new RegistreTestContextManager(clazz);
	}

	/**
	 * Get the {@link TestContextManager} associated with this runner.
	 */
	protected final RegistreTestContextManager getTestContextManager() {
	...
	protected void invokeTestMethod(Method method, RunNotifier notifier) {
		...
		new RegistreSpringMethodRoadie(getTestContextManager(), testInstance, testMethod, notifier, description).run();
	}
