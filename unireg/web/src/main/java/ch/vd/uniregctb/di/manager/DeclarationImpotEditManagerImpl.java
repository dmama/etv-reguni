...
public class DeclarationImpotEditManagerImpl implements DeclarationImpotEditManager, MessageSourceAware {
	...
	public void annulerDelai(DeclarationImpotDetailView diEditView, Long idDelai) {
		...
		Iterator<DelaiDeclaration> itDelai = delais.iterator();
		while (itDelai.hasNext()) {
			DelaiDeclaration delai = itDelai.next();
	...
	public String envoieImpressionLocalDuplicataDI(DeclarationImpotImpressionView diImpressionView) throws DeclarationException {
		...
		Iterator<ModeleDocumentView> itModeles = modeles.iterator();
		while (itModeles.hasNext()) {
			ModeleDocumentView modeleView = itModeles.next();
	...
	public void findByNumero(Long numero, DeclarationImpotListView diListView) {
		...
		Iterator<DeclarationImpotOrdinaire> itDI = dis.iterator();
		while (itDI.hasNext()) {
			DeclarationImpotOrdinaire di = itDI.next();
			...
			diView.setEtat(di.getDernierEtat().getEtat());
	...
	public void get(Long id, DeclarationImpotDetailView diEditView) {
		...
		diEditView.setId(id);
		EtatDeclaration etatDI = di.getDernierEtat();
		diEditView.setEtat(etatDI.getEtat());
		...
		if(etatDI.getEtat().equals(TypeEtatDeclaration.EMISE)) {
	...
	private void setDelais(DeclarationImpotDetailView diEditView, DeclarationImpotOrdinaire di) {
		...
		Iterator<DelaiDeclaration> itDelai = di.getDelais().iterator();
		while ( itDelai.hasNext() ) {
			DelaiDeclaration delai = itDelai.next();
	...
	public DeclarationImpotImpressionView getView(Long id, String typeDocument) {
		...
		Iterator<ModeleDocument> itModeles = modelesDocument.iterator();
		while(itModeles.hasNext()) {
			ModeleDocument modele = itModeles.next();
			if (!modele.getTypeDocument().equals(TypeDocument.LISTE_RECAPITULATIVE)
				...
				Iterator<ModeleFeuilleDocument> itModelesFeuilles = modele.getModelesFeuilleDocument().iterator();
				while (itModelesFeuilles.hasNext()) {
					ModeleFeuilleDocument modeleFeuilleDocument = itModelesFeuilles.next();
