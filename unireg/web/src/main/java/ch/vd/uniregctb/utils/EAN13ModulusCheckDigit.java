 ...
 * @since Validator 1.4
 */
public abstract class EAN13ModulusCheckDigit implements EAN13CheckDigit {

    private final int modulus;

    /**
     * Construct a {@link EAN13CheckDigit} routine for a specified modulus.
     *
     * @param modulus The modulus value to use for the check digit calculation
     */
    public EAN13ModulusCheckDigit(int modulus) {
    ...
    public boolean isValid(String code) {
        ...
        } catch (EAN13CheckDigitException  ex) {
            return false;
        }
    }
     ...
     * @throws EAN13CheckDigitException if an error occurs calculating the modulus
     * for the specified code
     */
    protected int calculateModulus(String code, boolean includesCheckDigit) throws EAN13CheckDigitException {
        int total = 0;
        ...
        if (total == 0) {
            throw new EAN13CheckDigitException("Invalid code, sum is zero");
        }
     ...
     * @throws EAN13CheckDigitException if character is non-numeric
     */
     ...
     * @throws EAN13CheckDigitException if integer character value
    ...
    protected String toCheckDigit(int charValue)
        ...
        if (charValue >= 0 && charValue <= 9) {
            ...
            throw new EAN13CheckDigitException("Invalid Check Digit Value =" +
                    + charValue);
        }
    }
