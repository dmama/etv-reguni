...
import ch.vd.uniregctb.stats.StatsData;
import ch.vd.uniregctb.stats.StatsService;
import ch.vd.uniregctb.tiers.Tiers;
import ch.vd.uniregctb.tiers.TiersDAO;
import ch.vd.uniregctb.utils.UniregProperties;
import ch.vd.uniregctb.webservice.sipf.BVRPlusClient;
import ch.vd.uniregctb.webservices.tiers.impl.TiersWebServiceTracing;
...
public class InfoController extends ParameterizableViewController implements AjaxHandler, InitializingBean {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		fillIndexeurInfo(mav);
		fillWSTiersInfo(mav);
		fillWSTiers2Info(mav);
	...
	private void fillServiceCivilInfo(ModelAndView mav) {
		...
		final StatsData stats = StatsService.getInfo(ServiceCivilService.SERVICE_NAME);
		mav.addObject("serviceCivilStats", stats);
	}
	...
	private void fillServiceInfraInfo(ModelAndView mav) {
		...
		final StatsData stats = StatsService.getInfo(ServiceInfrastructureService.SERVICE_NAME);
		mav.addObject("serviceInfraStats", stats);
	}
	...
	private void fillServiceSecuriteInfo(ModelAndView mav) {
		...
		final StatsData stats = StatsService.getInfo(ServiceSecuriteService.SERVICE_NAME);
		mav.addObject("serviceSecuriteStats", stats);
	}
	...
	private void fillIndexeurInfo(ModelAndView mav) {
		mav.addObject("indexeurStatus", "OK");
		final StatsData stats = StatsService.getInfo(GlobalIndexTracing.SERVICE_NAME);
		mav.addObject("indexeurStats", stats);
	}

	private void fillWSTiersInfo(ModelAndView mav) {
		mav.addObject("wsTiersStatus", "OK");
		final StatsData stats = StatsService.getInfo(TiersWebServiceTracing.SERVICE_NAME);
		mav.addObject("wsTiersStats", stats);
	}

	private void fillWSTiers2Info(ModelAndView mav) {
		mav.addObject("wsTiers2Status", "OK");
		final StatsData stats = StatsService.getInfo(ch.vd.uniregctb.webservices.tiers2.impl.TiersWebServiceTracing.SERVICE_NAME);
		mav.addObject("wsTiers2Stats", stats);
	}
