...
public class SituationFamilleManagerImpl extends TiersManager implements SituationFamilleManager {
	...
	public SituationFamilleView create(Long numeroCtb) throws AdressesResolutionException {
		...
		if ((contribuable instanceof PersonnePhysique || contribuable instanceof MenageCommun) && SecurityProvider.isGranted(Role.SIT_FAM)
			...
			if (contribuable instanceof MenageCommun) {
				...
				final List<String> nomCourrierPrincipal = adresseService.getNomCourrier(ppPrincipal, null);
				...
				if (ppConjoint != null) {
					...
					final List<String> nomCourrierConjoint = adresseService.getNomCourrier(ppConjoint, null);
	...
	public SituationFamille save(SituationFamilleView situationFamilleView) {
		...
		if (situationFamilleView.getNatureSituationFamille() == SITUATION_FAMILLE_MENAGE_COMMUN) {
