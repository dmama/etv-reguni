...
public class TiersWebServiceImpl implements TiersWebService {
	...
	private Map<Long, Object> mapTiers(Set<Long> tiersNumbers, RegDate date, Set<TiersPart> parts, MapCallback callback) {
		...
		final int nbThreads = Math.max(1, Math.min(20, allIds.size() / 25)); // un thread pour chaque multiple de 25 tiers. Au minimum 1 thread, au maximum 20 threads
		...
		if (totalTime > 0) {
			LOGGER.info(String.format("temps d'exécution: chargement des tiers=%d%%, mapping des tiers=%d%%", loadTiersTime * 100 / totalTime, mapTiersTime * 100 / totalTime));
		}
