	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors)
		...
		if (getTarget() != null) {
			if (TARGET_ANNULER_DELAI.equals(getTarget())) {
				...
				if (pcl == null) {
					throw new EditiqueCommunicationException("La communication avec l'éditique a échoué. Veuillez imprimer un duplicata de la liste récapitulative");
				}
			...
			if (request.getParameter(BUTTON_SAUVER) != null) {
				...
				byte[] pcl = lrEditManager.recoitImpressionLocal(docID);
				if (pcl == null) {
					throw new EditiqueCommunicationException("La communication avec l'éditique a échoué. Veuillez réessayer plus tard");
				}
				printPCLManager.openPclStream(request, response, pcl);
			}
