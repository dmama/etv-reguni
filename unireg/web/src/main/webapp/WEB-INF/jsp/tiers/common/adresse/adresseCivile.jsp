...
<display:table name="${adresses}" id="adresseCivile" pagesize="10" requestURI="${url}" class="display">
		<display:column  sortable ="true" titleKey="label.utilisationAdresse">
			<c:if test="${adresseCivile.annule}"><strike></c:if>
				<fmt:message key="option.usage.civil.${adresseCivile.usage}" />
			<c:if test="${adresseCivile.annule}"></strike></c:if>
		</display:column>
		...
		<display:column style="action">
			...
			<c:if test="${page == 'edit' }">
				<c:if test="${!adresseCivile.annule && adresseCivile.source !='CIVILE' && adresseCivile.id!= null && adresseCivile.id!='' }">
					<c:if test="${((adresseCivile.usage == 'COURRIER') && (command.allowedOnglet.ADR_C)) ||
					((adresseCivile.usage == 'POURSUITE') && (command.allowedOnglet.ADR_P)) ||
					((adresseCivile.usage == 'REPRESENTATION') && (command.allowedOnglet.ADR_B)) ||
					((adresseCivile.usage == 'DOMICILE') && (command.allowedOnglet.ADR_D))}">
						<!-- <a href="adresse.do?height=530&width=850&idAdresse=${adresseCivile.id}&numero=<c:out value="${command.tiers.numero}"></c:out>&TB_iframe=true&modal=true" class="thickbox edit" title="Edition d'adresse">&nbsp;</a> -->
						<unireg:raccourciAnnuler onClick="javascript:annulerAdresse(${adresseCivile.id});"/>
					</c:if>
				</c:if>
			</c:if>
		</display:column>
