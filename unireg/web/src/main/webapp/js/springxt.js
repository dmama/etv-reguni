...
/**
 * end of: initialise l'API springxt par les valeurs par défauts.
 */
...
XT.doAjaxAction = function(eventId, sourceElement, serverParams, clientParams) {
    ...
    while (url.charAt(url.length-1) === '#' || url.charAt(url.length-1) === '&') {
    	// msi (2009.02.20) : added trimming of trailing '&' to avoid tomcat warning about invalid chunk (Invalid chunk ignored)
    	url = url.substring(0, url.length-1);
    }
  	if (clientParams.clearQueryString && clientParams.clearQueryString == true && url.indexOf("?") != -1) {
...
XT.doAjaxSubmit = function(eventId, sourceElement, serverParams, clientParams) {
    ...
    // devacrfr correct bug : # in URL
    // BEGIN
    if ( clientParams.formName && document.forms[clientParams.formName].action !== "") {
    	ajaxClient = new XT.ajax.Client(document.forms[clientParams.formName].action);
    }
    // END
