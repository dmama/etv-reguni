...
public class TacheDAOTest extends CoreDAOTest {
	...
	public void testUpdateCollAdmAssignee() throws Exception {
		...
		doInNewTransaction(new TxCallback() {
			...
			public Object execute(TransactionStatus status) throws Exception {
				...
				PersonnePhysique ctb = addNonHabitant("Alfred", "Duah");
				ids.ctb = ctb.getNumero();

				PeriodeFiscale periode = addPeriode(2006);
	...
	private PeriodeFiscale addPeriode(int annee) {
		PeriodeFiscale periode = new PeriodeFiscale();
		periode.setAnnee(annee);
		periode = (PeriodeFiscale) hibernateTemplate.merge(periode);
		return periode;
	}
	...
	private DeclarationImpotOrdinaire addDeclaration(RegDate debut, RegDate fin, TypeContribuable typeCtb, Qualification qualif, PeriodeFiscale periode, ModeleDocument modele, Tiers tiers) {
		...
		di.setDateFin(fin);
		di.setTypeContribuable(typeCtb);
	...
	private PersonnePhysique addNonHabitant(String prenom, String nom) {
		PersonnePhysique ctb = new PersonnePhysique(false);
		ctb.setNom(nom);
		ctb.setPrenom(prenom);
		ctb = (PersonnePhysique) hibernateTemplate.merge(ctb);
		return ctb;
	}
	...
	private TacheTransmissionDossier addTacheTransmission(PersonnePhysique ctb, TypeEtatTache etat, CollectiviteAdministrative ca) {
		TacheTransmissionDossier transmission = new TacheTransmissionDossier();
		transmission.setContribuable(ctb);
		transmission.setCollectiviteAdministrativeAssignee(ca);
		transmission.setEtat(etat);
		transmission.setDateEcheance(date(2010, 1, 1));
		transmission = (TacheTransmissionDossier) hibernateTemplate.merge(transmission);
		return transmission;
	}

	private TacheNouveauDossier addTacheNouveau(PersonnePhysique ctb, TypeEtatTache etat, CollectiviteAdministrative ca) {
		TacheNouveauDossier nouveau = new TacheNouveauDossier();
		nouveau.setContribuable(ctb);
		nouveau.setCollectiviteAdministrativeAssignee(ca);
		nouveau.setEtat(etat);
		nouveau.setDateEcheance(date(2010, 1, 1));
		nouveau = (TacheNouveauDossier) hibernateTemplate.merge(nouveau);
		return nouveau;
	}

	private TacheControleDossier addTacheControle(PersonnePhysique ctb, TypeEtatTache etat, CollectiviteAdministrative ca) {
		TacheControleDossier controle = new TacheControleDossier();
		controle.setContribuable(ctb);
		controle.setCollectiviteAdministrativeAssignee(ca);
		controle.setEtat(etat);
		controle.setDateEcheance(date(2010, 1, 1));
	...
	private TacheAnnulationDeclarationImpot addTacheAnnulation(PersonnePhysique ctb, TypeEtatTache etat, CollectiviteAdministrative ca, DeclarationImpotOrdinaire di) {
		TacheAnnulationDeclarationImpot annulation = new TacheAnnulationDeclarationImpot();
		annulation.setContribuable(ctb);
		annulation.setDeclarationImpotOrdinaire(di);
		annulation.setCollectiviteAdministrativeAssignee(ca);
		annulation.setEtat(etat);
		annulation.setDateEcheance(date(2010, 1, 1));
		annulation = (TacheAnnulationDeclarationImpot) hibernateTemplate.merge(annulation);
		return annulation;
	}

	private TacheEnvoiDeclarationImpot addTacheEnvoi(PersonnePhysique ctb, TypeEtatTache etat, CollectiviteAdministrative ca) {
		TacheEnvoiDeclarationImpot envoi = new TacheEnvoiDeclarationImpot();
		envoi.setAdresseRetour(TypeAdresseRetour.CEDI);
		envoi.setContribuable(ctb);
		envoi.setCollectiviteAdministrativeAssignee(ca);
		envoi.setEtat(etat);
		envoi.setDateEcheance(date(2010, 1, 1));
		envoi.setDateDebut(date(2007, 1, 1));
		envoi.setDateFin(date(2007, 12, 31));
		envoi.setQualification(Qualification.AUTOMATIQUE);
		envoi.setTypeContribuable(TypeContribuable.VAUDOIS_ORDINAIRE);
		envoi.setTypeDocument(TypeDocument.DECLARATION_IMPOT_COMPLETE_BATCH);
