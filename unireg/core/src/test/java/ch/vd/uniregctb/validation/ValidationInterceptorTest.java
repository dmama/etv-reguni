...
public class ValidationInterceptorTest extends CoreDAOTest {
	...
	public void testValidationEntiteValide() throws Exception {
		...
		doInNewTransaction(new TransactionCallback() {
			public Object doInTransaction(TransactionStatus status) {
				...
				assertEmpty(jean.validate().getErrors());
				return null;
			}
		});
	}
		...
		try {
			doInNewTransaction(new TransactionCallback() {
				public Object doInTransaction(TransactionStatus status) {
					...
					final ValidationResults results = jean.validate();
					assertEquals(1, results.errorsCount());
					assertEquals("Le nom est un attribut obligatoire pour un non-habitant", results.getErrors().get(0));
					return null;
				}
			});
			fail("Une exception de validation aurait dû être levée.");
		}
		catch (ValidationException e) {
			final List<String> errors = e.getErrors();
			assertEquals(1, errors.size());
			assertEquals("Le nom est un attribut obligatoire pour un non-habitant", errors.get(0));
		}
	}
	...
	public void testValidationSousEntiteValideEtParentValide() throws Exception {
		...
		doInNewTransaction(new TransactionCallback() {
			public Object doInTransaction(TransactionStatus status) {
				...
				final ForFiscalPrincipal ffp = addForPrincipal(jean, date(2004, 3, 3), MotifFor.ARRIVEE_HC, null, null, 12345, TypeAutoriteFiscale.COMMUNE_OU_FRACTION_VD, MotifRattachement.DOMICILE);
				assertEmpty(ffp.validate().getErrors());
				assertEmpty(jean.validate().getErrors());
				return null;
			}
		});
			...
			doInNewTransaction(new TransactionCallback() {
				public Object doInTransaction(TransactionStatus status) {
					...
					final ForFiscalPrincipal ffp =
							addForPrincipal(jean, date(2004, 3, 3), MotifFor.ARRIVEE_HC, null, null, 12345, TypeAutoriteFiscale.COMMUNE_OU_FRACTION_VD, MotifRattachement.DOMICILE);
					assertEmpty(ffp.validate().getErrors()); // la sous-entité elle-même est valide
					return null;
				}
			});
			fail("Une exception de validation aurait dû être levée.");
		...
		try {
			doInNewTransaction(new TransactionCallback() {
				public Object doInTransaction(TransactionStatus status) {
					...
					assertEmpty(rapport.validate().getErrors()); // la join-entité elle-même est valide
