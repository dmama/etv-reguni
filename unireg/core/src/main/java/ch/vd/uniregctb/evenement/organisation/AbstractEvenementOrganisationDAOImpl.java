...
public abstract class AbstractEvenementOrganisationDAOImpl<EVT, TYP_EVT extends Enum<TYP_EVT>> extends BaseDAOImpl<EVT, Long> {

	public AbstractEvenementOrganisationDAOImpl(Class<EVT> persistentClass) {
	 ...
	 */
	protected String buildCriterion(Map<String, Object> criteria, EvenementOrganisationCriteria<TYP_EVT> criterion) {
		String queryWhere = "";
	...
	protected List<EVT> genericFind(final EvenementOrganisationCriteria<TYP_EVT> criterion, @Nullable final ParamPagination paramPagination) {
		...
		final String select = String.format("select evenement from %s evenement %s where 1=1 %s", getEvenementOrganisationClass().getSimpleName(), fromComplement, queryWhere);
