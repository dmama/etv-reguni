...
public class CodeCollectiviteUserType extends GenericUserType implements UserType {
	...
	@Override
	public Class<RegpmCodeCollectivite> returnedClass() {
		return RegpmCodeCollectivite.class;
	}

	@Override
	public RegpmCodeCollectivite nullSafeGet(ResultSet rs, String[] names, SessionImplementor session, Object owner) throws HibernateException, SQLException {
		final String name = names[0];
		final String code = StringUtils.trimToNull(rs.getString(name));
		if (code != null && !rs.wasNull()) {
			final RegpmCodeCollectivite s = MAPPING.get(code);
			if (s == null) {
