...
public class SexeUserType extends GenericUserType implements UserType {
	...
	@Override
	public Class<Sexe> returnedClass() {
		return Sexe.class;
	}

	@Override
	public Sexe nullSafeGet(ResultSet rs, String[] names, SessionImplementor session, Object owner) throws HibernateException, SQLException {
		final String name = names[0];
		final String code = StringUtils.trimToNull(rs.getString(name));
		if (code != null && !rs.wasNull()) {
			final Sexe s = MAPPING.get(code);
			if (s == null) {
