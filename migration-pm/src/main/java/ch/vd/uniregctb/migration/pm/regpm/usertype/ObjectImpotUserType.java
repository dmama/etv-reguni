...
public class ObjectImpotUserType extends GenericUserType implements UserType {
	...
	@Override
	public Class<RegpmObjectImpot> returnedClass() {
		return RegpmObjectImpot.class;
	}

	@Override
	public RegpmObjectImpot nullSafeGet(ResultSet rs, String[] names, SessionImplementor session, Object owner) throws HibernateException, SQLException {
		final String name = names[0];
		final String code = StringUtils.trimToNull(rs.getString(name));
		if (code != null && !rs.wasNull()) {
			final RegpmObjectImpot s = MAPPING.get(code);
			if (s == null) {
