...
public class ModelMBeanAssembler {
    ...
    public ModelMBeanInfo getModelMbeanInfo(Class<?> clazz) {
	    ...
	    for (ManagedNotification mn : mns) {
		    supporter.addModelMBeanNotification(mn.notificationTypes(),
				    mn.name(),
				    mn.description(), null);
	    }
	    ...
	    for (Method method1 : methods) {
		    ManagedAttribute ma = getManagedAttribute(method1);
		    //add Attribute to the ModelMBean
		    if (ma != null) {
			    String attributeName = getAttributeName(method1.getName());
			    ...
			    ManagedOperation mo = getManagedOperation(method1);

			    if (mo != null) {
				    Class<?>[] types = method1.getParameterTypes();
				    ManagedOperationParameter[] mop = getManagedOperationParameters(method1);
				    String[] paramTypes = new String[types.length];
				    String[] paramNames = new String[types.length];
				    String[] paramDescs = new String[types.length];
				    ...
				    Descriptor operationDescriptor =
						    supporter.buildOperationDescriptor(mo, method1.getName());
				    supporter.addModelMBeanMethod(method1.getName(),
						    paramTypes,
						    paramNames,
						    paramDescs,
						    mo.description(),
						    method1.getReturnType().getName(),
						    operationDescriptor);
			    }
		    }
