...
public final class ManagementConsole {
    ...
    public void getManagedObjectAttributes(ObjectName name) throws Exception {
	    ...
	    for (MBeanAttributeInfo attr : attrs) {
		    if (attr.isReadable()) {
			    try {
				    Object o = mbsc.getAttribute(name, attr.getName());
				    System.out.println("\t\t" + attr.getName() + " = " + o);
			    }
			    catch (Exception e) {
				    e.printStackTrace();
			    }
    ...
    ObjectName getEndpointObjectName() 
	    ...
	    buffer.append(ManagementConstants.SERVICE_NAME_PROP + "=\"").append(serviceName).append("\",");
	    buffer.append(ManagementConstants.PORT_NAME_PROP + "=\"").append(portName).append("\",*");
