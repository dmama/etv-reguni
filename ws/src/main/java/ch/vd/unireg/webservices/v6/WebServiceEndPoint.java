...
public class WebServiceEndPoint implements WebService, DetailedLoadMonitorable {
	...
	private static final Logger READ_ACCESS_LOG = LoggerFactory.getLogger("ws.v6.read");
	private static final Logger WRITE_ACCESS_LOG = LoggerFactory.getLogger("ws.v6.write");
	...
	private final ch.vd.unireg.ws.deadline.v6.ObjectFactory deadlineObjectFactory = new ch.vd.unireg.ws.deadline.v6.ObjectFactory();
	private final ch.vd.unireg.ws.taxoffices.v6.ObjectFactory taxOfficesObjectFactory = new ch.vd.unireg.ws.taxoffices.v6.ObjectFactory();
	private final ch.vd.unireg.ws.modifiedtaxpayers.v6.ObjectFactory modifiedTaxPayersFactory = new ch.vd.unireg.ws.modifiedtaxpayers.v6.ObjectFactory();
	private final ch.vd.unireg.ws.debtorinfo.v6.ObjectFactory debtorInfoFactory = new ch.vd.unireg.ws.debtorinfo.v6.ObjectFactory();
	...
	public Response searchParty(final String user, final String partyNo, final String name, final SearchMode nameSearchMode, final String townOrCountry,
		...
		return execute(user, params, READ_ACCESS_LOG, userLogin -> {
			...
			ch.vd.unireg.ws.search.party.v6.SearchResult result;
			try {
				...
				result = new ch.vd.unireg.ws.search.party.v6.SearchResult(null, infos);
			}
			catch (IndexerException e) {
				result = new ch.vd.unireg.ws.search.party.v6.SearchResult(new Error(ErrorType.BUSINESS, WebServiceHelper.buildExceptionMessage(e)), null);
			}
